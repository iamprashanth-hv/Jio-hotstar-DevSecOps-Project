pipeline{
    ...
    stages {
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){   
                       ...
                       sh "docker tag hotstar manojkrishnappa/hotstar:latest "
                       sh "docker push manojkrishnappa/hotstar:latest "
                    }
                }
            }
        }
        stage("TRIVY"){
            steps{
                sh "trivy image manojkrishnappa/hotstar:latest > trivyimage.txt"
            }
        }
        stage('Deploy to container'){
            steps{
                sh 'docker run -d --name hotstar -p 3000:3000 manojkrishnappa/hotstar:latest'
            }
        }
